...
def eventName_eventCount_eventCountPerUser(data):
    ...
    # First chart - Contagem de Eventos
    fig1 = go.Figure()
    fig1.add_trace(go.Bar(
        x=events,
        y=event_counts,
        text=[str(round(val, 2)) if pd.notnull(val) else '' for val in event_counts],
        textposition='outside',
        marker_color='rgb(55, 83, 109)',
        hovertemplate='Evento: %{x}<br>Contagem: %{y:.2f}<extra></extra>'
    ))
    ...
    fig1.update_layout(
        ...
        margin=dict(t=100, b=100)  # Added margin for labels
    )

    st.plotly_chart(fig1, use_container_width=True)
    st.divider()

    # Second chart - Contagem de Eventos por Usuário
    fig2 = go.Figure()
    fig2.add_trace(go.Bar(
        x=events,
        y=event_counts_per_user,
        text=[str(round(val, 2)) if pd.notnull(val) else '' for val in event_counts_per_user],
        textposition='outside',
        marker_color='rgb(50, 205, 50)',
        hovertemplate='Evento: %{x}<br>Contagem por Usuário: %{y:.2f}<extra></extra>'
    ))

    fig2.update_layout(
        title='Contagem de Eventos por Usuário',
        xaxis_title='Eventos',
        yaxis_title='Contagem por Usuário',
        showlegend=False,
        height=700,  # Increased height
        xaxis=dict(
            tickangle=45,
...
def session_over_dayOfWeeks(data):
    ...
    # Correct day order
    day_order = ['Domingo', 'Sábado', 'Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira']
    ...
    days = data['dayOfWeek'].unique().tolist()
    sessions = data.groupby('dayOfWeek')['sessions'].sum().reindex(day_order).round(2).tolist()
    engagedSessions = data.groupby('dayOfWeek')['engagedSessions'].sum().reindex(day_order).round(2).tolist()
    averageSessionDuration = data.groupby('dayOfWeek')['averageSessionDuration'].mean().reindex(day_order).round(2).tolist()
    engagementRate = data.groupby('dayOfWeek')['engagementRate'].mean().reindex(day_order).round(2).tolist()
    bounceRate = data.groupby('dayOfWeek')['bounceRate'].mean().reindex(day_order).round(2).tolist()
    screenPageViews = data.groupby('dayOfWeek')['screenPageViews'].sum().reindex(day_order).round(2).tolist()
    eventCount = data.groupby('dayOfWeek')['eventCount'].sum().reindex(day_order).round(2).tolist()
